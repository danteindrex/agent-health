...
export async function waitTimePredictor(args) {
    ...
    if (!ALLOW_DEMO_MOCKS) {
        throw new Error(`Wait time predictor is not implemented for production use${location_id ? ` for location ${location_id}` : ""}.`);
    }
...
export async function hitlPauseTrigger(args) {
    ...
    if (!ALLOW_DEMO_MOCKS) {
        throw new Error(`HITL pause is not implemented for production use. Reason received: ${reason}. Payload present: ${Boolean(payload)}.`);
    }
