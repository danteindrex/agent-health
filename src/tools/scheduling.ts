...
export async function availabilityQuery(args: any) {
  const { location_id, start } = args;
  // FHIR logic to search for Slots
  try {
    const searchParams: any = { status: "free" };
    if (location_id) searchParams["schedule.actor"] = `Location/${location_id}`;
    if (start) searchParams["start"] = `ge${start}`;
...
export async function appointmentBook(args: any) {
